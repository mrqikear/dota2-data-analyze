// Dota2 hero id -> internal name mapping (used for CDN image URLs)
const HERO_NAMES = {
...
export function heroName(id) {
  return HERO_NAMES[id] || 'default'
}

/**
 * Get hero icon URL from local cache (falls back to CDN on cache miss)
 */
export function heroIconUrl(id) {
  const name = heroName(id)
  return `/asset/hero/${name}.png`
}

/**
 * Get hero icon URL from CDN directly (no local cache)
 */
export function cdnHeroIconUrl(id) {
  const name = heroName(id)
  return `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/dota_react/heroes/icons/${name}.png`
}

/**
 * Get item icon URL from local cache
 */
...
/**
 * Get ability icon URL from local cache
 */
export function abilityIconUrl(name) {
  return `/asset/ability/${name}.png`
}
